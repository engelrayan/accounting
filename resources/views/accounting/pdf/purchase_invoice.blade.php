@php
    ...
    $settings = $purchaseInvoice->_pdfSettings ?? null;
...
<head>
    <meta charset="UTF-8">
    <title>فاتورة مشتريات {{ $purchaseInvoice->invoice_number }}</title>
    <link rel="stylesheet" href="{{ public_path('css/report-export.css') }}">
    <style>
        .inv-doc { border: 1px solid #dbe1ea; border-radius: 8px; padding: 28px; }
        .inv-header { display: table; width: 100%; margin-bottom: 20px; border-bottom: 2px solid #7c3aed; padding-bottom: 16px; }
        .inv-header > div { display: table-cell; vertical-align: top; }
        .inv-header > div:last-child { text-align: left; }
        .inv-company { font-size: 20px; font-weight: 700; color: #7c3aed; margin-bottom: 4px; }
        .inv-company-sub { font-size: 11px; color: #6b7280; }
        .inv-type { font-size: 13px; color: #7c3aed; font-weight: 600; margin-bottom: 4px; }
        ...
        .inv-status { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 600;
                      background: #ede9fe; color: #7c3aed; margin-top: 4px; }
        ...
        table.inv-items th { background: #7c3aed; color: #fff; padding: 8px 10px; font-size: 11px; text-align: right; }
        ...
        .inv-total-grand { font-size: 15px; font-weight: 700; color: #7c3aed; border-top: 2px solid #7c3aed !important; border-bottom: none !important; padding-top: 8px; }
        .inv-total-danger { color: #dc2626; }
        .inv-total-success { color: #16a34a; }
        .inv-notes { margin-top: 20px; padding: 12px; background: #f8fafc; border-radius: 6px; border-right: 3px solid #7c3aed; }
...
<div class="re-shell">
    ...
    <div class="inv-doc">
        ...
        <div class="inv-header">
            ...
            <div>
                <div class="inv-type">فاتورة مشتريات</div>
                <div class="inv-number">{{ $purchaseInvoice->invoice_number }}</div>
                <div class="inv-status">{{ $purchaseInvoice->statusLabel() }}</div>
            </div>
        </div>
        ...
        <div class="inv-info">
            <div>
                <div class="inv-label">من المورد</div>
                <div class="inv-party-name">{{ $purchaseInvoice->vendor->name }}</div>
                @if($purchaseInvoice->vendor->phone)
                    <div class="inv-party-detail">{{ $purchaseInvoice->vendor->phone }}</div>
                @endif
                @if($purchaseInvoice->vendor->email)
                    <div class="inv-party-detail">{{ $purchaseInvoice->vendor->email }}</div>
                @endif
                @if($purchaseInvoice->vendor_invoice_number)
                    <div class="inv-party-detail">رقم فاتورة المورد: {{ $purchaseInvoice->vendor_invoice_number }}</div>
                @endif
            </div>
            <div class="inv-dates">
                <div class="inv-date-row">
                    <span>تاريخ الإصدار</span>
                    <span>{{ $purchaseInvoice->issue_date->format('Y/m/d') }}</span>
                </div>
                @if($purchaseInvoice->due_date)
                <div class="inv-date-row">
                    <span>تاريخ الاستحقاق</span>
                    <span>{{ $purchaseInvoice->due_date->format('Y/m/d') }}</span>
                </div>
                @endif
                @if($purchaseInvoice->payment_method)
                <div class="inv-date-row">
                    <span>طريقة الدفع</span>
                    <span>{{ $purchaseInvoice->paymentMethodLabel() }}</span>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="inv-footer">
            ...
            <div class="inv-totals">
                ...
                <div class="inv-total-row">
                    ...
                    <span>{{ $fmt($purchaseInvoice->subtotal) }}</span>
                </div>
                <div class="inv-total-row">
                    <span>{{ $taxName }} ({{ rtrim(rtrim(number_format((float)$purchaseInvoice->tax_rate, 2),'0'),'.') }}%)</span>
                    <span>{{ $fmt($purchaseInvoice->tax_amount) }}</span>
                </div>
                @endif
                <div class="inv-total-row inv-total-grand">
                    <span>الإجمالي</span>
                    <span>{{ $fmt($purchaseInvoice->amount) }}</span>
                </div>
                <div class="inv-total-row">
                    <span>المدفوع</span>
                    <span class="inv-total-success">{{ $fmt($purchaseInvoice->paid_amount) }}</span>
                </div>
                <div class="inv-total-row inv-total-grand {{ $purchaseInvoice->remaining_amount > 0 ? 'inv-total-danger' : 'inv-total-success' }}">
                    <span>المتبقي</span>
                    <span>{{ $fmt($purchaseInvoice->remaining_amount) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="inv-notes">
            ...
            <div class="inv-notes-text">{{ $purchaseInvoice->notes }}</div>
        </div>
        @endif

        {{-- Footer --}}
        <div class="inv-footer-stamp">
            <div>تم الاستلام بواسطة<br>{{ $companyName }}</div>
            <div>{{ $purchaseInvoice->invoice_number }}<br>{{ now()->format('Y-m-d') }}</div>
            <div>توقيع المورد<br>&nbsp;</div>
        </div>
