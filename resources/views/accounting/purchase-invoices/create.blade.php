...
<div class="ac-page-header">
    <h1 class="ac-page-header__title">إنشاء فاتورة مشتريات جديدة</h1>
...
@endphp
<form method="POST" action="{{ route('accounting.purchase-invoices.store') }}" id="inv-form">
    @csrf
    ...
    <div class="ac-inv-create-grid">
        ...
        <div class="ac-inv-create-main">
            ...
            {{-- رقم الفاتورة (read-only) --}}
            <div class="ac-card ac-card--compact">
                <div class="ac-card__body">
                    <div class="ac-inv-number-preview">
                        <span class="ac-inv-number-preview__label">رقم الفاتورة الداخلي</span>
                ...
                <div class="ac-card__body">
                    <div class="ac-form-row">
                        <div class="ac-form-group">
                            <label class="ac-label ac-label--required" for="vendor_id">المورد</label>
                            <select id="vendor_id" name="vendor_id"
                                    class="ac-select {{ $errors->has('vendor_id') ? 'ac-select--error' : '' }}"
                                    required>
                                <option value="">— اختر المورد —</option>
                                @foreach($vendors as $v)
                                    <option value="{{ $v->id }}"
                                        {{ (old('vendor_id', $preselectedVendor) == $v->id) ? 'selected' : '' }}>
                                        {{ $v->name }}
                                        @if($v->phone) ({{ $v->phone }}) @endif
                                    </option>
                                @endforeach
                            </select>
                            @error('vendor_id') <span class="ac-field-error">{{ $message }}</span> @enderror
                        </div>

                        <div class="ac-form-group">
                            <label class="ac-label" for="vendor_invoice_number">رقم فاتورة المورد</label>
                            <input id="vendor_invoice_number" name="vendor_invoice_number" type="text"
                                   class="ac-control {{ $errors->has('vendor_invoice_number') ? 'ac-control--error' : '' }}"
                                   value="{{ old('vendor_invoice_number') }}"
                                   placeholder="الرقم الذي على فاتورة المورد (اختياري)">
                            @error('vendor_invoice_number') <span class="ac-field-error">{{ $message }}</span> @enderror
                        </div>

                        <div class="ac-form-group">
                            <label class="ac-label ac-label--required" for="issue_date">تاريخ الفاتورة</label>
                        ...
                        <div class="ac-inv-item-row" data-row="0">
                            ...
                            <div class="ac-inv-item__desc-wrap">
                                ...
                                <button type="button"
                                        ...
                                        onclick="openCatalog(this, 'purchase')">
...
(function () {
    ...
    const taxRateInp  = document.getElementById('tax_rate_input');
    let   rowCount    = 1;
    ...
    function rowTemplate(idx) {
        ...
        <div class="ac-inv-item__desc-wrap">
            <input type="text" name="items[${idx}][description]" class="ac-control ac-inv-item__desc" placeholder="وصف المنتج أو الخدمة" required>
            <button type="button" class="ac-inv-item__catalog-btn ac-btn ac-btn--ghost ac-btn--xs" title="اختر من الكتالوج" onclick="openCatalog(this, 'purchase')">
...
(function () {
    ...
    function renderResults(products) {
        ...
        resultsEl.innerHTML = products.map(p => {
            const price = priceField === 'purchase'
                ? parseFloat(p.purchase_price != null ? p.purchase_price : p.sale_price || 0)
