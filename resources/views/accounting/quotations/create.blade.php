...
<div style="font-family:'Cairo',sans-serif;direction:rtl;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.25rem;">
        ...
        <h1 style="margin:0;font-size:1.25rem;font-weight:700;color:#1e293b;">عرض سعر جديد</h1>
    </div>
    ...
    <div style="background:linear-gradient(135deg,#1e40af 0%,#4f46e5 100%);border-radius:16px;padding:1.75rem 2rem;margin-bottom:1.5rem;color:#fff;">
        <h2 style="margin:0;font-size:1.4rem;font-weight:800;">إنشاء عرض سعر جديد</h2>
        <p style="margin:.35rem 0 0;opacity:.85;font-size:.92rem;">أدخل بيانات العرض وأضف البنود المطلوبة</p>
    </div>
    ...
    <form method="POST" action="{{ route('accounting.quotations.store') }}" id="quotationForm">
        @csrf
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;margin-bottom:1.25rem;">
            ...
            <div style="background:#fff;border-radius:14px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,.05);padding:1.5rem;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:.85rem;margin-bottom:1rem;">
                    <div>
                        <label style="display:block;font-size:.85rem;font-weight:600;color:#374151;margin-bottom:.4rem;">
                            تاريخ العرض <span style="color:#ef4444;">*</span>
                        </label>
                    ...
                    <div>
                        <label style="display:block;font-size:.85rem;font-weight:600;color:#374151;margin-bottom:.4rem;">صالح حتى</label>
                        <input type="date" name="valid_until"
                               value="{{ old('valid_until') }}"
                               style="width:100%;padding:.55rem .85rem;border:1px solid #d1d5db;border-radius:8px;font-family:'Cairo',sans-serif;font-size:.9rem;color:#1e293b;box-sizing:border-box;">
                        <p style="margin:.3rem 0 0;font-size:.76rem;color:#6b7280;">اتركه فارغاً = بدون تاريخ انتهاء</p>
                    </div>
                </div>
            ...
            <div style="background:#fff;border-radius:14px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,.05);padding:1.5rem;">
                ...
                <div style="margin-bottom:1.25rem;">
                    <label style="display:block;font-size:.85rem;font-weight:600;color:#374151;margin-bottom:.4rem;">الشروط والأحكام</label>
                    <textarea name="terms" rows="5" placeholder="الشروط والأحكام..."
                              style="width:100%;padding:.55rem .85rem;border:1px solid #d1d5db;border-radius:8px;font-family:'Cairo',sans-serif;font-size:.9rem;color:#1e293b;resize:vertical;box-sizing:border-box;">{{ old('terms') }}</textarea>
                </div>

                {{-- Info box --}}
                <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:.85rem 1rem;display:flex;align-items:flex-start;gap:.6rem;">
                    <span style="color:#2563eb;font-size:1.1rem;line-height:1.3;">ℹ</span>
                    <p style="margin:0;font-size:.83rem;color:#1e40af;line-height:1.5;">
                        رقم العرض سيُولَّد تلقائياً بصيغة <strong>QT-2026-XXXX</strong> عند الحفظ
                    </p>
                </div>
            </div>
        </div>
        ...
        <div style="background:#fff;border-radius:14px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,.05);margin-bottom:1.25rem;">
            <div style="display:flex;align-items:center;justify-content:space-between;padding:1.25rem 1.5rem;border-bottom:1px solid #e2e8f0;">
                ...
                <button type="button" onclick="addRow()"
                        style="display:inline-flex;align-items:center;gap:.4rem;padding:.45rem 1rem;background:#4f46e5;color:#fff;border:none;border-radius:8px;font-family:'Cairo',sans-serif;font-size:.85rem;font-weight:600;cursor:pointer;">
                    + إضافة بند
                </button>
            </div>
                ...
                <table style="width:100%;border-collapse:collapse;" id="itemsTable">
                    <thead>
                        <tr style="background:#f8fafc;">
                            ...
                            <th style="padding:.6rem .75rem;text-align:center;font-size:.8rem;font-weight:700;color:#64748b;border-bottom:1px solid #e2e8f0;white-space:nowrap;width:70px;">الخصم%</th>
                    ...
                    <tbody id="itemsTbody">
                        ...
                        <tr data-idx="0">
                            ...
                            <td style="padding:.5rem .6rem;border-bottom:1px solid #f1f5f9;">
                                <input type="number" name="items[0][discount_rate]" class="discr" value="0" min="0" max="100" step="0.01"
                                       style="width:100%;padding:.4rem .55rem;border:1px solid #d1d5db;border-radius:6px;font-family:'Cairo',sans-serif;font-size:.82rem;text-align:center;box-sizing:border-box;">
                            </td>
                            <td style="padding:.5rem .6rem;border-bottom:1px solid #f1f5f9;">
            ...
            <div style="display:flex;justify-content:flex-end;padding:1rem 1.5rem 1.5rem;">
                <div style="min-width:260px;">
                    ...
                    <div style="display:flex;justify-content:space-between;padding:.4rem 0;font-size:.9rem;color:#475569;border-bottom:1px solid #f1f5f9;">
                        <span>الخصم:</span>
                        <span><span id="sumDiscount">0.00</span> ريال</span>
                    </div>
        ...
        <div style="display:flex;align-items:center;gap:.85rem;">
            <button type="submit"
                    style="padding:.6rem 1.75rem;background:#4f46e5;color:#fff;border:none;border-radius:9px;font-family:'Cairo',sans-serif;font-size:.95rem;font-weight:700;cursor:pointer;">
                حفظ كمسودة
            </button>
            <a href="{{ route('accounting.quotations.index') }}"
...
<template id="rowTemplate">
    <tr data-idx="__IDX__">
        ...
        <td style="padding:.5rem .6rem;border-bottom:1px solid #f1f5f9;">
            <input type="number" name="items[__IDX__][discount_rate]" class="discr" value="0" min="0" max="100" step="0.01"
                   style="width:100%;padding:.4rem .55rem;border:1px solid #d1d5db;border-radius:6px;font-family:'Cairo',sans-serif;font-size:.82rem;text-align:center;box-sizing:border-box;">
        </td>
        <td style="padding:.5rem .6rem;border-bottom:1px solid #f1f5f9;">
...
function addRow() {
    ...
    // Attach events to new row inputs
    newRow.querySelectorAll('.qty,.price,.taxr,.discr').forEach(el => el.addEventListener('input', recalc));
...
function recalc() {
    ...
    document.querySelectorAll('#itemsTbody tr').forEach((tr, i) => {
        ...
        const discR = parseFloat(tr.querySelector('.discr').value) || 0;
        const base  = qty * price;
        const d     = base * discR / 100;
        const t     = (base - d) * taxR / 100;
    ...
    document.getElementById('sumTotal').textContent    = (subtotal - disc + tax).toFixed(2);
}
...
document.querySelectorAll('#itemsTbody .qty,.price,.taxr,.discr').forEach(el => {
