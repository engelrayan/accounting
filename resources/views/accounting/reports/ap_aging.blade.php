...
@section('topbar-actions')
    <div class="ac-topbar-actions">
        <a href="{{ route('accounting.reports.ap-aging', array_filter(['as_of' => $as_of, 'export' => 'pdf'])) }}"
        ...
        <a href="{{ route('accounting.vendors.index') }}"
           class="ac-btn ac-btn--secondary ac-btn--sm">الموردون</a>
    </div>
@endsection
...
<div class="ac-aging-hero">
    <div class="ac-aging-hero__content">
        <span class="ac-aging-hero__eyebrow">الذمم الدائنة</span>
        <h2 class="ac-aging-hero__title">تقرير التقادم — AP Aging</h2>
        <p class="ac-aging-hero__text">
            يُظهر إجمالي المبالغ المستحقة لكل مورد موزعةً على شرائح زمنية بحسب عمر التأخر.
        </p>
    </div>
...
<div class="ac-card ac-report-filter-card">
    <div class="ac-card__body">
        <form method="GET" action="{{ route('accounting.reports.ap-aging') }}" class="ac-report-filter">
...
<div class="ac-dash-grid ac-dash-grid--4" style="margin-bottom:24px">
    <div class="ac-dash-card">
        ...
        <div class="ac-dash-card__footer">{{ $vendor_count }} مورد</div>
    </div>
    ...
    <div class="ac-dash-card {{ $total_overdue > 0 ? 'ac-dash-card--unbalanced' : '' }}">
        ...
        <div class="ac-dash-card__footer">{{ $overdueRatio }}% من الإجمالي</div>
    </div>
    ...
    <div class="ac-dash-card {{ $max_days_overdue > 90 ? 'ac-dash-card--unbalanced' : '' }}">
        ...
        <div class="ac-dash-card__footer">
            @if($max_days_overdue > 90) تحتاج إجراء فوري
            @elseif($max_days_overdue > 30) تحتاج متابعة
            @elseif($max_days_overdue > 0) ضمن المعقول
            @else لا تأخيرات
            @endif
        </div>
    </div>
    ...
    <div class="ac-empty">
        ...
        <div>
            ...
            <p style="font-size:13px;color:var(--ac-text-muted);margin:4px 0 0">
                جميع الفواتير مدفوعة أو لم تُسجَّل بعد.
            </p>
        </div>
    </div>
@else
    <div class="ac-card">
        <div style="overflow-x:auto">
            <table class="ac-table ac-aging-table">
                <thead>
                    <tr>
                        <th class="ac-aging-table__col-name">المورد</th>
                    ...
                    @foreach($rows as $row)
                        <tr class="ac-aging-row">
                            <td class="ac-aging-row__name">
                                <a href="{{ route('accounting.vendors.show', $row->vendor_id) }}"
                ...
                <tfoot>
                    <tr class="ac-aging-totals-row">
                        ...
                        <td class="ac-aging-cell ac-aging-cell--current"><strong>{{ $fmt($totals->current) }}</strong></td>
                        <td class="ac-aging-cell ac-aging-cell--30"><strong class="{{ $totals->b1_30 > 0 ? 'ac-text-warning' : '' }}">{{ $fmt($totals->b1_30) }}</strong></td>
                        <td class="ac-aging-cell ac-aging-cell--60"><strong class="{{ $totals->b31_60 > 0 ? 'ac-text-danger' : '' }}">{{ $fmt($totals->b31_60) }}</strong></td>
                        <td class="ac-aging-cell ac-aging-cell--90"><strong class="{{ $totals->b61_90 > 0 ? 'ac-text-danger' : '' }}">{{ $fmt($totals->b61_90) }}</strong></td>
                        <td class="ac-aging-cell ac-aging-cell--91plus"><strong class="{{ $totals->b91plus > 0 ? 'ac-text-danger' : '' }}">{{ $fmt($totals->b91plus) }}</strong></td>
                        <td class="ac-aging-cell ac-aging-cell--total"><strong>{{ $fmt($totals->total) }}</strong></td>
                    </tr>
                </tfoot>
            </table>
        </div>
