...
@section('title', 'كشف حساب مورد — ' . $vendor->name)
...
@section('topbar-actions')
    <div class="ac-topbar-actions">
        <a href="{{ route('accounting.vendors.show', $vendor->id) }}"
           class="ac-btn ac-btn--secondary ac-btn--sm">صفحة المورد</a>
        <button type="button" class="ac-btn ac-btn--secondary ac-btn--sm"
                onclick="window.print()">طباعة</button>
        <a href="{{ route('accounting.reports.vendor-statement', array_merge([$vendor->id], ['from' => $from, 'to' => $to, 'export' => 'pdf'])) }}"
...
<div class="ac-stmt-header">
    <div class="ac-stmt-header__customer">
        ...
        <div>
            <h2 class="ac-stmt-header__name">{{ $vendor->name }}</h2>
...
<div class="ac-card ac-report-filter-card ac-print-hide">
    <div class="ac-card__body">
        <form method="GET" action="{{ route('accounting.reports.vendor-statement', $vendor->id) }}"
...
<div class="ac-dash-grid ac-dash-grid--4" style="margin-bottom:24px">
    <div class="ac-dash-card">
        ...
        <div class="ac-dash-card__footer">
            @if($opening_balance > 0) مستحق للمورد
            @elseif($opening_balance < 0) رصيد لصالحنا
            @else مُسوَّى @endif
        </div>
    </div>
    ...
    <div class="ac-dash-card">
        ...
        <div class="ac-dash-card__amount ac-text-danger">{{ $fmt($total_credit) }}</div>
        <div class="ac-dash-card__footer">
    ...
    <div class="ac-dash-card">
        <div class="ac-dash-card__header">
            <span class="ac-dash-card__label">إجمالي المدفوعات</span>
        ...
        <div class="ac-dash-card__amount ac-text-success">{{ $fmt($total_debit) }}</div>
        <div class="ac-dash-card__footer">
        ...
        <table class="ac-table ac-stmt-table">
            <thead>
                <tr>
                    ...
                    <th class="ac-stmt-col-num">دفعة (مدين)</th>
                    <th class="ac-stmt-col-num">فاتورة (دائن)</th>
                ...
                @forelse($transactions as $txn)
                    <tr class="ac-stmt-row ac-stmt-row--{{ $txn->type }}">
                        ...
                        <td class="ac-stmt-cell-ref">
                            @if($txn->type === 'invoice')
                                <span class="ac-stmt-ref-link">{{ $txn->reference }}</span>
                            @else
                        ...
                        <td class="ac-stmt-cell-num">
                            @if($txn->debit > 0)
                                <span class="ac-stmt-credit">{{ $fmt($txn->debit) }}</span>
                            @else
                                —
                            @endif
                        </td>
                        <td class="ac-stmt-cell-num">
                            @if($txn->credit > 0)
                                <span class="ac-stmt-debit">{{ $fmt($txn->credit) }}</span>
                            @else
                                —
                            @endif
                        </td>
            ...
            <tfoot>
                <tr class="ac-stmt-totals-row">
                    ...
                    <td class="ac-stmt-cell-num">
                        <strong class="ac-text-success">{{ $fmt($total_debit) }}</strong>
                    </td>
                    <td class="ac-stmt-cell-num">
                        <strong class="ac-text-danger">{{ $fmt($total_credit) }}</strong>
                    </td>
                    <td></td>
                </tr>
                <tr class="ac-stmt-closing-row">
                    ...
                    <td class="ac-stmt-cell-balance ac-stmt-closing-val {{ $closing_balance > 0 ? 'ac-text-danger' : ($closing_balance < 0 ? 'ac-text-success' : 'ac-text-muted') }}">
                        ...
                        @if($closing_balance > 0)
                            <span class="ac-stmt-closing-badge ac-stmt-closing-badge--debit">مستحق للمورد</span>
                        @elseif($closing_balance < 0)
                            <span class="ac-stmt-closing-badge ac-stmt-closing-badge--credit">رصيد لصالحنا</span>
                        @else
