...
<div style="display:flex;gap:8px;align-items:center;">
    @if($balance > 0)
        ...
        @endcan
    @endif
    <a href="{{ route('accounting.reports.vendor-statement', $vendor->id) }}"
    ...
    <a href="{{ route('accounting.vendors.index') }}" class="ac-btn ac-btn--secondary">رجوع</a>
</div>
@endsection
...
@endif
@if(session('info'))
    <div class="ac-alert ac-alert--info" data-dismiss="alert">{{ session('info') }}</div>
@endif

{{-- ── Profile Header ─────────────────────────────────────────────────────── --}}
<div class="ac-cust-profile-header">
    <div class="ac-cust-avatar ac-cust-avatar--lg">{{ $vendor->initial() }}</div>
    <div class="ac-cust-profile-info">
        <h1 class="ac-cust-profile-name">{{ $vendor->name }}</h1>
...
<div class="ac-dash-grid">
    ...
    <div class="ac-dash-card">
        <div class="ac-dash-card__header">
            <span class="ac-dash-card__label">إجمالي المشتريات</span>
        ...
        <div class="ac-dash-card__footer">{{ $vendor->purchaseInvoices->count() }} فاتورة</div>
    </div>

    <div class="ac-dash-card">
        <div class="ac-dash-card__header">
            <span class="ac-dash-card__label">إجمالي المدفوع</span>
    ...
    <div class="ac-dash-card {{ $balance > 0 ? 'ac-dash-card--alert' : '' }}">
        <div class="ac-dash-card__header">
            <span class="ac-dash-card__label">المستحق للمورد</span>
        ...
        <div class="ac-dash-card__footer">
            @if($balance > 0)
                <span class="ac-text-danger">مستحق للمورد</span>
            @elseif($balance < 0)
                رصيد دائن لنا
            @else
...
<div class="ac-balance-banner">
    ...
    <div class="ac-balance-banner__body">
        ...
        <span>للمورد مبلغ <strong class="ac-text-danger">{{ number_format($balance, 2) }}</strong> لم يُسدَّد بعد.</span>
    </div>
    @can('can-write')
...
<div class="ac-partner-actions" id="pay-panel">
    ...
    <div class="ac-card">
        <div class="ac-card__body">
            <p class="ac-section-label">إنشاء فاتورة مشتريات</p>
            <p class="ac-text-muted" style="font-size:.85rem;margin-bottom:12px;">
                تسجيل فاتورة شراء جديدة من هذا المورد مع بنود تفصيلية.
            </p>
            <a href="{{ route('accounting.purchase-invoices.create', ['vendor_id' => $vendor->id]) }}"
    ...
    <div class="ac-card">
        <div class="ac-card__body">
            <p class="ac-section-label">تسجيل دفعة للمورد</p>
            <form method="POST" action="{{ route('accounting.vendors.payments.store', $vendor) }}">
                @csrf
        ...
        <table class="ac-table">
            ...
            <tbody>
                ...
                <tr class="{{ $invoice->isOverdue() ? 'ac-row--overdue' : 'ac-row--unpaid' }}">
                    <td class="ac-text-mono ac-font-bold">
                        <a href="{{ route('accounting.purchase-invoices.show', $invoice) }}" class="ac-table-link">
                            {{ $invoice->invoice_number }}
                        </a>
                    </td>
                    <td class="ac-table__muted">{{ $invoice->vendor_invoice_number ?: '—' }}</td>
                    ...
                    <td class="ac-table__num ac-text-danger ac-font-bold">{{ number_format($invoice->remaining(), 2) }}</td>
                    <td>
            ...
            <tfoot>
                <tr class="ac-table__foot-total">
                    <td colspan="4" class="ac-font-bold">مجموع غير المدفوع</td>
                    <td class="ac-table__num ac-font-bold">{{ number_format($unpaidInvoices->sum(fn($i) => (float) $i->amount), 2) }}</td>
                    <td class="ac-table__num ac-font-bold ac-text-success">{{ number_format($unpaidInvoices->sum(fn($i) => $i->totalPaid()), 2) }}</td>
                    <td class="ac-table__num ac-font-bold ac-text-danger">{{ number_format($unpaidInvoices->sum(fn($i) => $i->remaining()), 2) }}</td>
                    <td></td>
                </tr>
            </tfoot>
        </table>
...
<h2 class="ac-section-title ac-mt-4">
    ...
    <span class="ac-count-pill">{{ $vendor->purchaseInvoices->count() }}</span>
</h2>

@if($vendor->purchaseInvoices->isEmpty())
    <div class="ac-empty-state ac-empty-state--sm ac-text-muted">لم تُسجَّل أي فواتير مشتريات بعد.</div>
@else
<div class="ac-card">
    <div class="ac-card__body" style="padding:0;">
        <table class="ac-table">
            ...
            <tbody>
                @foreach($vendor->purchaseInvoices as $invoice)
                <tr class="{{ $invoice->isOverdue() ? 'ac-row--overdue' : ($invoice->isUnpaid() ? 'ac-row--unpaid' : '') }}">
                    ...
                    <td class="ac-table__num">{{ number_format($invoice->amount, 2) }}</td>
                    <td class="ac-table__num ac-text-success">{{ number_format($invoice->totalPaid(), 2) }}</td>
                    <td class="ac-table__num {{ $invoice->remaining() > 0 ? 'ac-text-danger ac-font-bold' : '' }}">{{ number_format($invoice->remaining(), 2) }}</td>
                    <td><span class="ac-badge ac-badge--{{ $invoice->statusMod() }}">{{ $invoice->statusLabel() }}</span></td>
                </tr>
                @endforeach
            </tbody>
            <tfoot>
                <tr class="ac-table__foot-total">
                    ...
                    <td class="ac-table__num ac-font-bold {{ $balance > 0 ? 'ac-text-danger' : '' }}">{{ number_format(max(0, $totalInvoiced - $totalPaid), 2) }}</td>
                    <td></td>
                </tr>
            </tfoot>
        </table>
...
<h2 class="ac-section-title ac-mt-4">
    ...
    سجل المدفوعات
    <span class="ac-count-pill ac-count-pill--green">{{ $allPayments->count() }}</span>
</h2>

@if($allPayments->isEmpty())
    <div class="ac-empty-state ac-empty-state--sm ac-text-muted">لم يتم تسجيل أي مدفوعات بعد.</div>
@else
<div class="ac-card">
    <div class="ac-card__body" style="padding:0;">
        <table class="ac-table">
            ...
            <tfoot>
                <tr class="ac-table__foot-total">
                    <td colspan="2" class="ac-font-bold">إجمالي المدفوع</td>
                    <td class="ac-table__num ac-font-bold ac-text-danger">
...
<div id="settle-modal" class="ac-modal" role="dialog" aria-modal="true" aria-labelledby="settle-modal-title" hidden>
    ...
    <div class="ac-modal__dialog">
        ...
        <div class="ac-modal__header">
            ...
            <div>
                <h2 class="ac-modal__title" id="settle-modal-title">تسوية حساب المورد</h2>
                <p class="ac-modal__subtitle">{{ $vendor->name }}</p>
            </div>
        ...
        <div class="ac-modal__body">
            <p class="ac-modal__message">
                هل أنت متأكد من تسوية حساب هذا المورد؟<br>
                سيتم اعتبار جميع الفواتير المستحقة مدفوعةً وتسجيل قيد محاسبي تلقائي.
            </p>
        ...
        <div class="ac-modal__footer">
            <button type="button" class="ac-btn ac-btn--secondary" data-modal-close="settle-modal">إلغاء</button>
            <form method="POST" action="{{ route('accounting.vendors.settle', $vendor) }}"
